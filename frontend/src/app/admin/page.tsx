...
function FootballEventEditor({ match, event, onChanged, isNew = false }: { match: Match; event?: FootballEvent; onChanged: () => Promise<void>; isNew?: boolean }) {
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      const body = { teamId, playerId, eventType, half, minute, extraTimeMinute };
  ...
  const fields = <><Field label="Event"><Select value={eventType} onChange={(e) => setEventType(e.target.value as FootballEvent["event_type"])}><option value="goal">Goal</option><option value="yellow_card">Yellow card</option><option value="red_card">Red card</option><option value="substitution">Substitution</option></Select></Field><Field label="Team"><Select value={teamId} onChange={(e) => chooseTeam(Number(e.target.value))}><option value={match.teamA.id}>{match.teamA.shortName}</option><option value={match.teamB.id}>{match.teamB.shortName}</option></Select></Field><Field label="Player" className="sm:col-span-2"><Select value={playerId} onChange={(e) => setPlayerId(Number(e.target.value))}><option value={0}>Choose player</option>{(team.players || []).map((entry) => <option key={entry.playerId} value={entry.playerId}>#{entry.jerseyNumber} {entry.player.name}</option>)}</Select></Field><Field label="Half"><Input type="number" min={1} max={2} value={half} onChange={(e) => setHalf(Number(e.target.value))} /></Field><Field label="Minute"><Input type="number" min={0} max={120} value={minute} onChange={(e) => setMinute(Number(e.target.value))} /></Field><Field label="Added minute" className="sm:col-span-2"><Input type="number" min={0} max={30} value={extraTimeMinute} onChange={(e) => setExtra(Number(e.target.value))} /></Field></>;
