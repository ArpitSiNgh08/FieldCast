...
export default async function HomePage() {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-6 sm:py-10">
    ...
    {tournaments.length > 0 && <section className="mb-8" aria-label="Approved tournaments"><h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-muted">Tournaments</h2><div className="-mx-4 flex gap-4 overflow-x-auto px-4 pb-2 sm:mx-0 sm:grid sm:grid-cols-2 sm:overflow-visible sm:px-0 lg:grid-cols-3">{tournaments.map((tournament) => <Link key={tournament.id} href={`/tournaments/${tournament.id}`} aria-label={`Open ${tournament.name}`} className="group w-[82vw] max-w-sm shrink-0 overflow-hidden rounded-xl border border-border bg-surface shadow-sm transition-all hover:-translate-y-0.5 hover:border-accent hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60 sm:w-auto sm:max-w-none"><img src={tournament.imageUrl || "/tournament-placeholder.svg"} alt="" className="h-36 w-full object-cover transition-transform group-hover:scale-[1.02]" /><div className="p-4"><p className="text-xs font-medium uppercase tracking-wide text-muted">{SPORT_LABEL[tournament.sport]}</p><h3 className="mt-1 font-semibold text-foreground">{tournament.name}</h3><p className="mt-2 text-xs text-muted">{tournament.teams.length} teams · {tournament.format || "Format to be announced"}</p></div></Link>)}</div></section>}
