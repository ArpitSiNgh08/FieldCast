...
export default function FootballMatchControl() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      {match.status !== "completed" && <div className="mt-8 grid gap-4 lg:grid-cols-2">
        <details open={match.status === "upcoming"} className="group overflow-hidden rounded-xl border border-border bg-surface shadow-sm lg:col-span-1"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4 font-semibold marker:hidden"><span>Match setup, cameras and preflight</span><span className="text-sm text-muted transition-transform group-open:rotate-180">⌄</span></summary><div className="space-y-6 border-t border-border p-4">
          <details open={match.status === "upcoming"} className="group overflow-hidden rounded-xl border border-border bg-surface shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4 font-semibold marker:hidden"><span>1. Match details</span><span className="text-sm text-muted transition-transform group-open:rotate-180">⌄</span></summary><Card className="rounded-none border-0 shadow-none"><CardBody className="grid gap-4 border-t border-border sm:grid-cols-2"><Field label="Kickoff"><Input type="datetime-local" value={scheduledAt} onChange={(event) => setScheduledAt(event.target.value)} /></Field><Field label="Venue"><Input value={venue} onChange={(event) => setVenue(event.target.value)} /></Field><Button variant="outline" onClick={saveSetup} disabled={busy} className="sm:col-span-2">Save setup</Button></CardBody></Card></details>
        ...
        <details open={match.status === "live"} className="group overflow-hidden rounded-xl border border-border bg-surface shadow-sm lg:col-span-1"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4 font-semibold marker:hidden"><span>Live scorecard and broadcast</span><span className="text-sm text-muted transition-transform group-open:rotate-180">⌄</span></summary><div className="space-y-6 border-t border-border p-4">
