...
function TeamEditor({ tournament, team, poolId, savedPlayers, refresh, refreshPlayers, setError }: { tournament: Tournament; team: Tournament["teams"][number]["team"]; poolId: number | null; savedPlayers: Player[]; refresh: () => Promise<void>; refreshPlayers: () => void; setError: (value: string) => void }) {
  const [name, setName] = useState(team.name); const [short, setShort] = useState(team.shortName); const [selectedPoolId, setSelectedPoolId] = useState(poolId ? String(poolId) : "");
  ...
  async function saveTeam() { setBusy(true); try { await api.updateTournamentTeam(tournament.id, team.id, { name, shortName: short, poolId: selectedPoolId ? Number(selectedPoolId) : null }); await refresh(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not update team"); } finally { setBusy(false); } }
