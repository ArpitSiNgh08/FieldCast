...
|---|---|
| System architecture, deployment phases (1 vs 2), tech stack, DB schema, real-time data flow | `README.md` |
...
- **Prisma 7 adapter is mandatory.** Every `new PrismaClient()` call requires a pg driver adapter (`@prisma/adapter-pg`). The bare constructor with no arguments or with `datasourceUrl`/`datasources` will throw. See `src/config/prisma.js` for the singleton pattern to copy.
...
- **Update PROGRESS.md as you go.** If you complete, start, or block on a task, reflect it there so the next agent (or Arpit) has accurate context without re-reading the whole conversation history.
