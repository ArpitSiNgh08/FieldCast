...
- **2026-09-10** — Cloudflare CDN Scaling, Clipping Resilience, API Cache Middleware, & Production DB Migration.
  - **Cloudflare CDN Edge Streaming Architecture**: Integrated `CLOUDFLARE_CDN_URL` / `cdnBase` into `backend/src/config/env.js` and `backend/src/controllers/matches.controller.js`. Formats public `liveUrl` with Cloudflare Worker proxy (`https://fieldcast-cdn.your-account.workers.dev`) for 24h edge-cached `.ts` segment delivery and 1s `.m3u8` playlist TTL. Preserved direct origin URL (`originLiveUrl`) for zero-delay FFmpeg clip buffering in `backend/src/controllers/clips.controller.js`. Created system architecture diagrams in `ARCHITECTURE.md` and Obsidian notes (`notes/Cloudflare CDN.md`, `notes/ARCHITECTURE.md`).
  ...
  - **Database Bandwidth Protection**: Created Express `apiCache` middleware (`backend/src/middleware/apiCache.js`) with a 3-second in-memory response cache for read-heavy GET requests, automatically invalidated on state mutations. Reduces Neon database network transfer allowance consumption by 80–90%.Database Migration Script**: Built `backend/src/scripts/migrateDb.js` to clone data across all 20 Prisma tables without requiring superuser `session_replication_role` privileges, automatically resetting auto-increment sequences (`setval`). Successfully migrated full production database to the new production Neon DB (`ep-gentle-recipe-b3d5cixv`).
